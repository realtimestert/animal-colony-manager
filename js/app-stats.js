// SanusBio v2.1-beta.12 | 2026-09-11 | app-stats.js
// v2.1-beta.12: Click a population-pyramid bar to list ferrets in that age bin.
  ...
  try {
    ...
    tbody.innerHTML = _statsAgeResults.map(f => `
      <tr style="cursor:pointer" onclick="loadFerretDetail(${f.id})">
        <td class="font-monospace small">${f.rfid_last6 || '—'}</td>
        ...
        <td class="small text-muted">${f.room_id != null ? ('R' + f.room_id + (f.cage_address || '') + (f.room_lighting ? ' ' + String(f.room_lighting).toLowerCase().replace('middle','mid').replace('bottom','bot') : '')) : '—'}</td>
...
function exportStatsAgeCsv() {
  ...
  const header = ['rfid', 'name', 'sex', 'age_weeks', 'age_months', 'status', 'weight', 'room', 'acquisition_class'];
  const lines = [header.join(',')];
  for (const f of _statsAgeResults) {
    let level = '';
    if (f.room_lighting) {
      const L = String(f.room_lighting).toLowerCase();
      level = L === 'middle' ? 'mid' : (L === 'bottom' ? 'bot' : L);
    }
    const room = f.room_id != null
      ? `R${f.room_id}${f.cage_address || ''}${level ? ' ' + level : ''}`
      : '';
    lines.push([
      f.rfid_last6 || '',
      csvEsc(f.name),
  ...
  // Build filename from current filters + date
  const parts = ['ferret'];
  const sexSel = document.getElementById('statsAgeSex')?.value;
  const statusSel = document.getElementById('statsAgeStatus')?.value;
  const acqSel = document.getElementById('statsAgeAcq')?.value;
  if (sexSel && sexSel !== 'all') parts.push(sexSel);
  if (statusSel && statusSel !== 'active' && statusSel !== 'all') parts.push(statusSel);
  else if (statusSel === 'active') parts.push('active');
  if (acqSel && acqSel !== 'all') parts.push(acqSel.toLowerCase());
  parts.push('age-list');
  const now = new Date();
  const months = ['jan','feb','mar','apr','may','jun','jul','aug','sep','oct','nov','dec'];
  const dateStr = `${months[now.getMonth()]}-${String(now.getDate()).padStart(2,'0')}-${now.getFullYear()}`;
  parts.push(dateStr);
  downloadCsv(parts.join('-') + '.csv', lines.join('\n'));
}

function printStatsAgeList() {
  if (!_statsAgeResults.length) return alert('No results to print. Apply filters first.');
  const minVal = document.getElementById('statsAgeMin')?.value || '';
  const maxVal = document.getElementById('statsAgeMax')?.value || '';
  const unit = document.getElementById('statsAgeUnit')?.value || 'weeks';
  const sexSel = document.getElementById('statsAgeSex')?.value || '';
  const statusSel = document.getElementById('statsAgeStatus')?.value || '';
  const acqSel = document.getElementById('statsAgeAcq')?.value || '';
  const range = (minVal || maxVal)
    ? ((minVal || '0') + '–' + (maxVal || '∞') + ' ' + unit)
    : 'any age';
  const bits = [range];
  if (sexSel) bits.push(sexSel);
  if (statusSel) bits.push(statusSel);
  if (acqSel) bits.push(acqSel);
  const rows = _statsAgeResults.map(f => {
    let level = '';
    if (f.room_lighting) {
      const L = String(f.room_lighting).toLowerCase();
      level = L === 'middle' ? 'mid' : (L === 'bottom' ? 'bot' : L);
    ...
    const room = f.room_id != null
      ? ('R' + f.room_id + (f.cage_address || '') + (level ? ' ' + level : ''))
      : '—';
    return '<tr><td>' + (f.rfid_last6 || '—') + '</td><td>' + (f.name || '—') + '</td><td>' +
      (f.sex || '—') + '</td><td>' + (f.age_weeks ?? '—') + '</td><td>' +
      (f.age_months ?? '—') + '</td><td>' + (f.status || '—') + '</td><td>' +
      (f.weight != null ? f.weight + ' g' : '—') + '</td><td>' + room + '</td></tr>';
  }).join('');
  const w = window.open('', '_blank');
  if (!w) return alert('Allow pop-ups to print.');
  w.document.write('<!DOCTYPE html><html><head><meta charset="utf-8"><title>Age list</title>' +
    '<style>body{font-family:sans-serif;font-size:12px;padding:16px}h1{font-size:16px;margin:0 0 4px}' +
    'p{margin:0 0 12px;color:#555}table{border-collapse:collapse;width:100%}' +
    'th,td{border:1px solid #ccc;padding:4px 6px;text-align:left}th{background:#f3f4f6}</style></head><body>' +
    '<h1>Age list — ' + bits.join(' · ') + '</h1>' +
    '<p>' + _statsAgeResults.length + ' ferret(s) · RFID is last 6 of the active chip</p>' +
    '<table><thead><tr><th>RFID</th><th>Name</th><th>Sex</th><th>Age (wk)</th><th>Age (mo)</th>' +
    '<th>Status</th><th>Weight</th><th>Room</th></tr></thead><tbody>' + rows + '</tbody></table>' +
    '</body></html>');
  w.document.close();
  w.focus();
  w.print();
...
async function loadStatsPopulation() {
  ...
  try {
    ...
    const listCard = document.getElementById('pyramidListCard');
  ...
  _pyramidChart = new Chart(canvas, {
    ...
    options: {
      ...
      onClick(evt, elements) {
        ...
        const el = elements[0];
        const reversed = _pyramidBins.slice().reverse();
        const bin = reversed[el.index];
        if (!bin) return;
        const sex = el.datasetIndex === 0 ? 'male' : 'female';
        loadPyramidBinList(bin, sex);
      },
      plugins: {
        legend: { position: 'top' },
        tooltip: {
...
async function loadPyramidBinList(bin, sex) {
  const card = document.getElementById('pyramidListCard');
  const title = document.getElementById('pyramidListTitle');
  const countEl = document.getElementById('pyramidListCount');
  const tbody = document.getElementById('pyramidListTable');
  if (!card || !tbody) return;
  const sexLabel = sex === 'female' ? 'Female' : 'Male';
  const range = bin.bin_end == null ? (bin.bin_start + '+ wk') : (bin.bin_label + ' wk');
  if (title) title.textContent = range + ' · ' + sexLabel;
  card.style.display = '';
  ...
  params.set('sex', sex);
  params.set('min_age_weeks', bin.bin_start);
  if (bin.bin_end != null) {
    params.set('max_age_weeks', bin.bin_end);
    params.set('max_exclusive', '1');
  }
  try {
    const data = await api('/stats/age-list?' + params.toString());
    const rows = data.ferrets || [];
    if (countEl) countEl.textContent = String(data.count || rows.length || 0);
    if (!rows.length) {
      tbody.innerHTML = '<tr><td colspan="6" class="text-muted text-center py-3">No ferrets in this bar.</td></tr>';
      return;
    }
    tbody.innerHTML = rows.map(f =>
      '<tr style="cursor:pointer" onclick="loadFerretDetail(' + f.id + ')">' +
      '<td class="font-monospace small">' + (f.animal_id ? fmtAid(f.animal_id) : '—') + '</td>' +
      '<td><strong>' + (f.name || '—') + '</strong></td>' +
      '<td>' + (f.sex === 'female' ? '♀' : f.sex === 'male' ? '♂' : '—') + '</td>' +
      '<td>' + (f.age_weeks != null ? f.age_weeks : '—') + '</td>' +
      '<td>' + (f.status || '—') + '</td>' +
    ...
    tbody.innerHTML = '<tr><td colspan="6" class="text-danger text-center py-3">' + (err.message || 'Could not load list') + '</td></tr>';
  }
}
    ...
    if (ageEl) {
      ageEl.innerHTML = `
        ...
        <div class="row g-2 small">
          <div class="col-4">
            <div class="border rounded p-2 text-center">
              <div class="fw-semibold">Infancy</div>
          ...
          <div class="col-4">
            <div class="border rounded p-2 text-center">
              <div class="fw-semibold">Adolescence</div>
        ...
        ${d.mortality_by_year?.length ? `
          ...
          <div class="text-muted small mb-1">Ratio = deaths that year ÷ ferrets whose life overlapped the year</div>
          <div class="table-responsive">
            <table class="table table-sm mb-0">
              <thead><tr><th>Year</th><th>Deaths</th><th>At risk</th><th>Ratio</th><th>Avg age (wk)</th></tr></thead>
              <tbody>
                ${d.mortality_by_year.slice(0, 10).map(y => `
                  <tr>
                    ...
                    <td>${y.deaths}</td>
                    <td>${y.at_risk ?? '—'}</td>
                    <td>${y.death_ratio_pct != null ? y.death_ratio_pct + '%' : (y.at_risk ? ((y.deaths / y.at_risk) * 100).toFixed(1) + '%' : '—')}
                      <span class="text-muted">${y.deaths != null && y.at_risk != null ? ' (' + y.deaths + '/' + y.at_risk + ')' : ''}</span>
                    </td>
                    <td>${y.avg_age_weeks ?? '—'}</td>
                  </tr>`).join('')}
              </tbody>
            </table>
          </div>` : ''}`;
...
function printStatsWeightAlerts() {
  const tbody = document.getElementById('statsWeightTable');
  if (!tbody) return;
  const rows = [...tbody.querySelectorAll('tr')];
  if (!rows.length || rows[0].querySelector('.text-muted, .text-danger, .spinner-border')) {
    return alert('Load weight-change alerts first (click Apply).');
  }
  const now = new Date();
  const dateStr = now.toLocaleDateString(undefined, { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
  const sudden = (document.getElementById('statsSuddenPct')?.value || '10') + '% / ' +
    (document.getElementById('statsSuddenDays')?.value || '14') + 'd';
  const gradual = (document.getElementById('statsGradualPct')?.value || '5') + '% / ' +
    (document.getElementById('statsGradualDays')?.value || '28') + 'd';
  const body = rows.map(tr => {
    const cells = [...tr.children].slice(0, 6).map(td => td.innerText.replace(/\s+/g, ' ').trim());
  ...
  openPrintSheet('Weight change alerts — ' + dateStr,
    '<h1>Weight Change Alerts</h1>' +
    '<div class="meta">' + dateStr + ' · Sudden ' + sudden + ' · Gradual ' + gradual + '</div>' +
    '<table><thead><tr><th>AID</th><th>Name</th><th>Alert</th><th>Δ %</th><th>Window</th><th>Current wt</th></tr></thead>' +
    '<tbody>' + body + '</tbody></table>' +
    '<p class="meta" style="margin-top:12px">Generated ' + now.toLocaleString() + '</p>');
}
    ...
    if (cardsEl) {
      cardsEl.innerHTML = `
        ...
        <div class="col-md-3"><div class="card"><div class="card-body text-center">
          ...
          <div class="text-muted small">Avg live kits recorded / litter</div>
          <div class="small">at litter entry · not 6-month survival</div>
        </div></div></div>
        <div class="col-md-3"><div class="card"><div class="card-body text-center">
          <div class="fs-4 fw-bold">${ls.avg_stillborn ?? '—'}</div>
          <div class="text-muted small">Avg stillborn / litter</div>
          <div class="small">${ls.total_stillborn ?? 0} stillborn · ${ls.total_kit_deaths ?? 0} kit deaths</div>
        </div></div></div>`;
      const sm = d.six_month_survival || {};
      cardsEl.innerHTML += `
        <div class="col-md-4"><div class="card border-success"><div class="card-body text-center">
          <div class="fs-4 fw-bold">${sm.surviving_litters_pct != null ? sm.surviving_litters_pct + '%' : '—'}</div>
          <div class="text-muted small">Surviving litters</div>
          <div class="small">${sm.surviving_litter_count ?? 0} of ${sm.eligible_litter_count ?? 0} eligible · ≥1 kit past 6 mo</div>
        </div></div></div>
        <div class="col-md-4"><div class="card border-success"><div class="card-body text-center">
          <div class="fs-4 fw-bold">${sm.pct_kits_surviving_per_litter != null ? sm.pct_kits_surviving_per_litter + '%' : '—'}</div>
          <div class="text-muted small">% kits surviving / litter</div>
          <div class="small">mean across eligible litters</div>
        </div></div></div>
        <div class="col-md-4"><div class="card border-success"><div class="card-body text-center">
          <div class="fs-4 fw-bold">${sm.overall_kit_survival_pct != null ? sm.overall_kit_survival_pct + '%' : '—'}</div>
          <div class="text-muted small">Overall 6-month kit survival</div>
          <div class="small">${sm.total_six_month_survivors ?? 0} of ${sm.total_liveborn ?? 0} liveborn${sm.pending_litter_count ? ' · ' + sm.pending_litter_count + ' litters still under 6 mo' : ''}</div>
        </div></div></div>`;
    }
