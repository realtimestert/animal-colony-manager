...
function switchReportTab(tab) {
  ...
  const nameWrap = document.getElementById('repDeathNameWrap');
  if (nameWrap) nameWrap.style.display = tab === 'deaths' ? '' : 'none';
  loadCurrentReport();
}
...
async function renderDeathsReport(el) {
  ...
  const q = (document.getElementById('repDeathSearch')?.value || '').trim();
  ...
  const filtered = needle
    ? rows.filter(f => {
        ...
        const aid = String(f.animal_id || '');
        const aidFmt = typeof fmtAid === 'function' ? String(fmtAid(f.animal_id) || '').toLowerCase() : '';
        return name.includes(needle) || aid.includes(needle) || aidFmt.includes(needle)
          || ('aid' + aid.padStart(5, '0')).includes(needle.replace(/\s/g, ''))
          || String(f.notes || '').toLowerCase().includes(needle);
      })
    : rows;
  el.innerHTML = `
    <div class="card">
      <div class="card-header bg-white py-3 d-flex align-items-center flex-wrap gap-2">
        ...
        <span class="text-muted small">${reportDateRangeLabel(from, to)}</span>
        ${q ? `<span class="text-muted small">· matching “${q.replace(/[<>&"]/g, '')}”</span>` : ''}
      </div>
      <div class="table-responsive">
        <table class="table table-hover mb-0">
          <thead><tr><th>Name</th><th>Age at Death (wk)</th><th>Date of Death</th><th>Cause</th><th>Notes</th></tr></thead>
          <tbody>${filtered.length ? filtered.map(f => `
            <tr>
              <td>
                <a href="#" class="text-decoration-none" onclick="event.preventDefault();loadFerretDetail(${f.id})">
                  <strong>${f.name}</strong>
                </a>
                <span class="text-muted small">${typeof fmtAid === 'function' ? (fmtAid(f.animal_id) || '') : (f.animal_id || '')}</span>
              </td>
              <td>${reportAgeWeeks(f.birth_date, f.death_date)}</td>
              <td>${fmtDate(f.death_date)}</td>
              <td class="small">${f.cause_of_death || '—'}</td>
              <td class="small" style="white-space:pre-wrap;max-width:420px;line-height:1.45">${f.notes || '—'}</td>
            </tr>`).join('') : `<tr><td colspan="5" class="text-muted text-center py-3">${q ? 'No deaths match that name or AID.' : 'No deaths in this range.'}</td></tr>`}
          </tbody>
        </table>
      </div>
    </div>`;
  ...
  el.innerHTML = `
    <div class="card mb-3">
      <div class="card-header bg-white py-3 d-flex align-items-center">
        ...
        <span class="text-muted small ms-2">${reportDateRangeLabel(from, to)} · stillborns and post-birth kit deaths are counted separately</span>
      </div>
      <div class="card-body">
        <div class="row g-3 text-center">
          <div class="col-6 col-md-3"><div class="fs-2 fw-bold">${totalStillborn}</div><div class="text-muted small">Stillborn (not kit deaths)</div></div>
